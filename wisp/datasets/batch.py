...
class ImageBatch(Batch):
    """ Represents a single batch of coordinates + pixel information sampled and collated from a WispDataset.
    ImageBatch objects are always assumed to carry the coordinate information and corresponding pixel values.
    Batches in Wisp keep a general structure by subclassing python's dictionaries and using their semantics,
    ...
    def __init__(self,
                 ...
                 *args, **kwargs):
        """
        Creates a new ImageBatchFull of coordinate samples and supervision signals they carry.

        Args:
            coords (torch.Tensor): a torch Tensor of coordinate samples of shape (B, 2) where B is the number of
        ...
        """
        super().__init__(coords=coords, rgb=rgb)

    def coord_values(self) -> Dict[str, Any]:
        """ Specifies a dictionary of the coordinate specific supervision channels this ImageBatch carries. """
        out = dict(rgb=self['rgb'])
        return out
