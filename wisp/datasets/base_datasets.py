...
from wisp.datasets.batch import Batch, MultiviewBatch, SDFBatch, ImageBatch
...
class BaseImageDataset(WispDataset):
    """ Extends the WispDataset with dataset behavior common to all image based datasets. """

    @property
    @abstractmethod
    def coordinates(self) -> torch.Tensor:
        """ Returns a tensor of image coordinates. """
        raise NotImplementedError('ImageDatasets should return a (N, 2) tensor of sample coordinates.')

    def __getitem__(self, idx: int) -> ImageBatch:
        """ Samples the dataset for a coordinate+rgb batch of information.
        The exact coordinate supervision channels the batch contains are up to the dataset implementation to determine.
        Callers may treat the batch as a dictionary, or query ImageBatch.coord_values() to view which
        supervision channels are available.
        """
        raise NotImplementedError('ImageDataset should override __getitem__')

    def __len__(self):
        """Length of the dataset as number of coordinate samples. """
        raise NotImplementedError('ImageDataset should override __len__')
