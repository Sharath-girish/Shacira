...
at::Tensor hashgrid_interpolate2d_cuda(
    at::Tensor coords,
    at::Tensor codebook,
    at::Tensor codebook_first_idx,
    std::vector<int32_t> resolution,
    ...
    for (int32_t i=0; i < resolution.size(); ++i) {
        hashgrid_interpolate2d_cuda_impl(num_coords, codebook_size, feature_dim, resolution[i], i, num_lods, coords, 
                                       codebook, codebook_first_idx, feats);
    }
    return feats;
#else
...
at::Tensor hashgrid_interpolate2d_backward_cuda(
    at::Tensor coords,
    at::Tensor grad_output,
    at::Tensor codebook,
    at::Tensor codebook_first_idx,
    ...
    at::Tensor grad_coords;
    if (require_grad_coords) {
        grad_coords = at::zeros({num_coords, 2}, coords.options());
    } else {
        grad_coords = at::empty({0}, coords.options());
    }

    for (int32_t i=0; i < resolution.size(); ++i) {
        hashgrid_interpolate2d_backward_cuda_impl(num_coords, codebook_size, feature_dim, 
                resolution[i], i, num_lods, require_grad_coords,
                coords, codebook, codebook_first_idx, grad_output, grad_codebook, grad_coords);
    }
    return grad_codebook;
